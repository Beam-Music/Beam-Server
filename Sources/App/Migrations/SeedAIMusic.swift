...
struct SeedAIMusic: AsyncMigration {
    ...
    func prepare(on database: Database) async throws {
        ...
        if let existingArtist = try await Artist.query(on: database)
            ...
            let newArtist = Artist(name: artistName, debutYear: 2024) // Artist 모델에 debutYear 필드가 있다고 가정
        ...
        let targetPlaylist: RecommendPlaylist // RecommendPlaylist 모델이 정의되어 있다고 가정
    ...
    func revert(on database: Database) async throws {
        ...
        if songIDsToDelete.isEmpty {
             ...
             try await aiSongsDeleteQuery.delete(force: true)

             let _: Void = try await Song.query(on: database)
